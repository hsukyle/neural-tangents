...
def denseActivationNormLayer(n_hidden_unit, activation, norm):
    if activation == 'relu':
        activation = Relu
    ...
    if norm is None:
        return stax.serial(Dense(n_hidden_unit), activation)
    elif norm == 'batch_norm':
        return stax.serial(Dense(n_hidden_unit), activation, BatchNorm(axis=0))
    else:
        raise ValueError

def mlp(n_output, n_hidden_layer, n_hidden_unit, activation='relu', norm=None):

    return stax.serial(
        *[denseActivationNormLayer(n_hidden_unit, activation, norm) for i in range(n_hidden_layer)],
        Dense(n_output)
    )
...
if __name__ == '__main__':
    ...
    for i, (x, y) in enumerate(minibatch(task['x_train'], task['y_train'], batch_size=256, train_epochs=1000)):
        ...
        if i == 0 or (i+1) % 100 == 0:
            print(f"train loss: {loss(get_params(opt_state), (task['x_train'], task['y_train']))},\ttest loss: {loss(get_params(opt_state), (task['x_test'], task['y_test']))}")
