...
every_iter_plot_keys = win_loss_keys + win_loss_eval_keys + win_rmse_keys + win_rmse_eval_keys + \
    win_tvd_keys + win_tvd_eval_keys
if args.dataset in ['omniglot', 'circle']:
...
for i, task_batch in tqdm(enumerate(taskbatch(task_fn=task_fn,
    ...
    aux = dict()
    # ntk
    if i == 0 or (i + 1) % (args.n_train_task // args.task_batch_size // 20) == 0:
    ...
    if i == 0 or (i + 1) % (args.n_train_task // args.task_batch_size // 20) == 0:
        plotter.log_to_line(
            ...
            xlabel='maml update',
            ylabel='rank',
            X=onp.arange(log['ntk_train_rank_eval'].shape[0]) * (args.n_train_task // args.task_batch_size // 20),
            plot_smooth=False
        )
